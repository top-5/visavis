import { DoubleSide, CustomBlending, SrcAlphaFactor, OneMinusSrcAlphaFactor } from 'three';

import * as THREE from 'three';

const HAIR_MATERIAL_CONFIG = {
  'Base Color': { r: 0, g: 5, b: 255 },
  'Specular 1 Power': 50,
  'Specular 2 Power': 100,
  'Tilt 1': 0.5,
  'Tilt 2': 2,
...
// Load a texture
const colorTexture = textureLoader.load('/textures/hair-blonde.png');

// Similarly, load other textures as needed
const alphaTexture = textureLoader.load('/textures/T_StandardWSet_Alpha.png');
const occlusionTexture = textureLoader.load('path/to/your/occlusionTexture.png');
const directionTexture = textureLoader.load('path/to/your/directionTexture.png');
const tiltTexture = textureLoader.load('/textures/tilt_texture.png');
const highlightTexture = textureLoader.load('/textures/highlight_texture.png');
...
export const hairStrandShader = {
  uniforms: {
    uColor: {
      value: {
        r: HAIR_MATERIAL_CONFIG['Base Color'].r / 255,
        g: HAIR_MATERIAL_CONFIG['Base Color'].g / 255,
        b: HAIR_MATERIAL_CONFIG['Base Color'].b / 255
      }
    },
    ...
    uColorText: { value: colorTexture },
    uAlphaText: { value: alphaTexture },
    uOccTexture: { value: 0 },
    uDirectionText: { value: 0 },
    uTiltText: { value: tiltTexture },
    uHighlightText: { value: 0 },

    uHasColorText: { value: true },
...
export const hairMaterial = new THREE.ShaderMaterial({
  uniforms: {
    ...THREE.UniformsUtils.clone(hairStrandShader.uniforms),
    uHasDirectionText: { value: false } // Add the missing value property
  },
  vertexShader: hairStrandShader.vertexShader,
  fragmentShader: hairStrandShader.fragmentShader,
  transparent: hairStrandShader.transparent,
  blending: hairStrandShader.blending,
  blendSrc: hairStrandShader.blendSrc,
  blendDst: hairStrandShader.blendDst,
  side: hairStrandShader.side,
  depthWrite: hairStrandShader.depthWrite,
  alphaTest: hairStrandShader.alphaTest
});
